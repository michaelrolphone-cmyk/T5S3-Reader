...
typedef struct {
    ...
    // returns. The firmware automatically relaunches the exact ELF path that made
    // the request, so callers do not need to know their filename. `cookie` is
    // opaque caller state returned unchanged with the result. max_length == 0
    // preserves the firmware keyboard's unlimited-length behavior.
