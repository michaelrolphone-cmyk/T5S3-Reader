 ...
 * Reject every undefined symbol outside the current public libc contract and
 * the exact versioned privileged OS/CPU inventory. No dynamic-module fallback
 * or hardware-specific firmware export is permitted for a physical provider.
 * Both .dynsym and .symtab are inspected if present. */
