...
**Scope authorization is mandatory before implementation.** An explicit current user request and constraints define the deliverable; the roadmap describes long-term possibilities and is NOT permission to implement every adjacent feature or upgrade a future feature into an MVP requirement. In each active PR/spec, write a short change contract stating the user objective, included deliverables, explicitly excluded/deferred features, acceptance tests, and basis for each blocking dependency. Classify additions as REQUIRED, OPTIONAL/nonblocking, or DEFERRED. A perceived best practice, speculative security concern, nearby roadmap item, existing experiment or agent preference is not authorization to expand scope. In particular, do not introduce a new cryptographic trust system, new persistent security policy, breaking package format, new hardware-ownership layer, cross-PR dependency or release gate unless it is directly needed for the request or expressly approved. Record separable enhancements for later instead. Audit the PR against its change contract before claiming completion; correct drift in specs and code without discarding useful independent work or weakening existing safety checks.

**Mandatory for current package-manager and driver-loader work:** read [docs/PACKAGE_MANAGER_SCOPE_CONTRACT.md](docs/PACKAGE_MANAGER_SCOPE_CONTRACT.md). Unified package management does not require publisher signatures or signed-only driver execution. Cryptographic signing, signer key provisioning/revocation, security-version floors and signed provenance are DEFERRED, not prerequisites for PR #76 or PR #78. Preserve bounded validation, payload integrity checks, safe file transactions and independently enforced loader permissions; a hash is not publisher authentication. The scope contract supersedes contrary MVP-level acceptance requirements in older package/roadmap documents until the user explicitly authorizes an expansion. Do not falsely claim that existing signed-code experiments have already been removed or decoupled.
...
**Trusted RiscRTE system UI mediation and package-private application storage are high priority and should be implemented ahead of new app-private substitutes, only within the explicit scope of the current task.**
...
- if the required picker/private-storage primitive is missing, use an existing suitable path within authorized scope or identify a separate prerequisite instead of silently expanding the task;
