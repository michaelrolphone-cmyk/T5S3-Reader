#!/usr/bin/env python3
"""Build the MSP430FR target-programming provider ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/program_msp"
OUTPUT = ROOT / "dist/experimental/program-msp"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "program-msp", "driver_abi": 2,
    ...
    if any(manifest.get(k) != v for k, v in required.items()):
        raise ValueError("Invalid MSP programmer manifest")
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("MSP programmer ELF must export only t5_driver_get")
