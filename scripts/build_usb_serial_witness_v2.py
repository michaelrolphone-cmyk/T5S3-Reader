#!/usr/bin/env python3
"""Build the U1 fourth-class installability witness ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/usb_serial_witness_v2"
OUTPUT = ROOT / "dist/experimental/usb-serial-witness"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-serial-witness", "driver_abi": 2,
        "architecture": "xtensa-esp32s3", "file_name": "driver.elf",
        "requires": [{"capability": "usb.host", "api": 1}],
        "provides": [{"capability": "serial.port", "api": 1}],
        "status": "u1-test-witness-unpublished",
    }
    if any(manifest.get(key) != value for key, value in required.items()):
        raise ValueError("Invalid U1 fourth-class witness manifest")
    ...
    if exported != {"t5_driver_get"}:
        raise ValueError("ABI-v2 witness must export only t5_driver_get")
    ...
    if not 52 <= len(payload) <= 256 * 1024 or payload[:7] != b"\x7fELF\x01\x01\x01" or \
       ...
       int.from_bytes(payload[18:20], "little") != 94:
        raise ValueError("Invalid Xtensa witness ELF")
    manifest.update(size_bytes=len(payload), sha256=hashlib.sha256(payload).hexdigest())
    (OUTPUT / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    print(f"U1 fourth-class serial witness built: {elf}; unpublished test provider")
    return elf
