#!/usr/bin/env python3
"""Build independently installable CH34x USB-to-serial class ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/usb_ch34x_v2"
OUTPUT = ROOT / "dist/experimental/usb-ch34x-v2"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-ch34x-v2", "driver_abi": 2,
    ...
    if any(manifest.get(k) != v for k, v in required.items()):
        raise ValueError("Invalid CH34x provider manifest")
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("CH34x ELF must export only t5_driver_get")
