...
SOURCE = ROOT / "Drivers/usb_cp210x_v2"
OUTPUT = ROOT / "dist/experimental/usb-cp210x-v2"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-cp210x-v2", "driver_abi": 2,
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("CP210x ELF must export only t5_driver_get")
