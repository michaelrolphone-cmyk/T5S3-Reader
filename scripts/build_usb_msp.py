#!/usr/bin/env python3
"""Build the installable TI MSP-FET/eZ-FET USB debug provider ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/usb_msp"
OUTPUT = ROOT / "dist/experimental/usb-msp"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-msp", "driver_abi": 2,
        "architecture": "xtensa-esp32s3", "file_name": "driver.elf",
        "requires": [
            {"capability": "usb.host", "api": 1},
            {"capability": "platform.clock", "api": 1},
        ],
        "provides": [{"capability": "debug.vendor.msp", "api": 1}],
        "status": "experimental-unpublished",
    }
    if any(manifest.get(k) != v for k, v in required.items()):
        raise ValueError("Invalid MSP-FET manifest")
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("MSP-FET ELF must export only t5_driver_get")
