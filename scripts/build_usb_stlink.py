#!/usr/bin/env python3
"""Build the installable ST-LINK V2/V2.1/V3 USB probe ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/usb_stlink"
OUTPUT = ROOT / "dist/experimental/usb-stlink"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-stlink", "driver_abi": 2,
        "architecture": "xtensa-esp32s3", "file_name": "driver.elf",
        "requires": [{"capability": "usb.host", "api": 1}, {"capability": "platform.clock", "api": 1}],
        "provides": [{"capability": "debug.vendor.stlink", "api": 1}],
        "status": "experimental-unpublished",
    }
    if any(manifest.get(k) != v for k, v in required.items()):
        raise ValueError("Invalid ST-LINK manifest")
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("ST-LINK ELF must export only t5_driver_get")
