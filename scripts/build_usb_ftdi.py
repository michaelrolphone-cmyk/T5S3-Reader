#!/usr/bin/env python3
"""Build the installable FTDI USB serial class ELF."""
import hashlib
import json
import os
...
SOURCE = ROOT / "Drivers/usb_ftdi"
OUTPUT = ROOT / "dist/experimental/usb-ftdi"


def build(cc=None):
    manifest = json.loads((SOURCE / "manifest.json").read_text())
    required = {
        "type": "driver", "id": "usb-ftdi", "driver_abi": 2,
    ...
    if any(manifest.get(k) != v for k, v in required.items()):
        raise ValueError("Invalid FTDI manifest")
    ...
    if exports != {"t5_driver_get"}:
        raise ValueError("FTDI ELF must export only t5_driver_get")
