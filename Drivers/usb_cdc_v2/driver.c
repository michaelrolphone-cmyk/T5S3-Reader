...
/* Never discard the class token or its grant on an ambiguous physical
 * release. A successful data release can be committed independently, then
 * retry the still-owned control claim with the exact same token. */
...
static void stop(void) {
    if (!host) return;
    /* Recovery/test stop cannot bypass physical release or unload while a
     * class token remains. Normal graph teardown calls quiesce first. */
    for (size_t i = 0; i < RISC_USB_CDC_MAX_SESSIONS; ++i)
...
/* Parse a complete configuration without trusting interface ordering. One
 * ACM function is supported, including unrelated composite interfaces, CDC
 * union, IAD, nonzero alternate. Ambiguous functions/alternates fail closed. */
...
static uint64_t open_device(uint64_t device) {
    ...
    if (!host->claim(host->context, device, candidate.control_interface, 0,
        ...
        return 0;
    /* Reserve an owned token BEFORE the second claim: failure of its cleanup
     * cannot turn the first physical claim into an untracked orphan. */
    ++generation;
