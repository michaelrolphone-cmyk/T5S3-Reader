...
void updateDeviceHandle(bool present) {
  ...
  if (deviceGeneration != UINT32_MAX) ++deviceGeneration;
...
t5_serial_result_t acquirePort(const t5_serial_port_request_t* request,
  ...
  if (!request || !lease || !rx || !tx || !validConfig(&request->config)) return T5_SERIAL_INVALID;
  ...
  if (leaseGeneration != UINT32_MAX) ++leaseGeneration;
...
t5_serial_result_t configurePort(t5_serial_port_lease_t lease, const t5_serial_config_t* config) {
  ...
  if (!validConfig(config)) return config && config->flow_control != T5_SERIAL_FLOW_NONE
      ? T5_SERIAL_UNSUPPORTED : T5_SERIAL_INVALID;
