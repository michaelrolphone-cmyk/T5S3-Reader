...
void renderTable(const t5_ui_chrome_t* chrome, const t5_ui_table_column_t* columns, uint32_t columnCount,
  ...
  const int desiredRowHeight = std::max(36, metrics.listRowHeight);
  ...
  const int rowHeight = visibleCount > 0 ? std::max(36, (available - headerHeight) / visibleCount) : desiredRowHeight;
...
int32_t hitTest(int16_t x, int16_t y) {
  ...
  if (y < hitLayout.rowTop) return T5_UI_HIT_HEADER;
  if (hitLayout.rowHeight <= 0 || hitLayout.pageItems <= 0) return T5_UI_HIT_NONE;
  ...
  *event = {};
  ...
  if (raw.tapped) {
    ...
    for (size_t i = 0; i < bounds.size(); ++i) {
      ...
      if (raw.touch_x < b.x || raw.touch_x >= b.x + b.width || raw.touch_y < b.y || raw.touch_y >= b.y + b.height) {
        continue;
      }
