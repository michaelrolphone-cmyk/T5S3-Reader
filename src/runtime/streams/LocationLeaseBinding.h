...
// Firmware-only integration with the single production device registry.
// The caller authenticates the execution-context owner, uses the driver-owning
// task and serializes every operation using the existing stream mutex. Poll GPS
// outside that mutex and submit only a copied observation. The provider lease
// is borrowed; the driver releases it after disconnect/teardown.
...
class LocationLeaseBinding final {
 public:
  ...
  int32_t attach(uint32_t owner, uint32_t device, uint32_t providerGrant, Token* out) {
    ...
    device_ = device; // Generation-safe physical handle, not model identity.
  ...
  // grant for each consumer, bound to this exact device generation.
  ...
  // Provider owning task only. Never trust a caller-provided epoch.
  ...
  // CapabilityLost/Removed or driver shutdown: accepted records remain
  // readable until DISCONNECTED. Release subscriber device grants immediately.
  // The borrowed source grant is NEVER released here.
  void disconnect() {
  ...
  // Invoke before Registry::release(owner) and driver ELF unload.
