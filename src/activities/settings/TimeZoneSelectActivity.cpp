...
bool TimeZoneSelectActivity::onTouchTap(int16_t x, int16_t y) {
...
void TimeZoneSelectActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Time Zone");
  const int y = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing * 3;
  if (launchFailed) {
    renderer.drawCenteredText(UI_10_FONT_ID, y, "time_zone.elf could not be launched");
    renderer.drawCenteredText(SMALL_FONT_ID, y + 36, "Install Time Zone from the App Store or copy it to /Apps.");
