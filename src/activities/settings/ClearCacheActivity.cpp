...
void ClearCacheActivity::loop() {
  if (!launchAttempted) {
    ...
    const esp_err_t result = runNativeApp("/sd/Apps/clear_cache.elf", renderer, mappedInput);
    if (result == ESP_OK) {
      finish();
      return;
    }
...
void ClearCacheActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Clear Reading Cache");
  const int y = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing * 3;
  if (launchFailed) {
    renderer.drawCenteredText(UI_10_FONT_ID, y, "clear_cache.elf could not be launched");
    renderer.drawCenteredText(SMALL_FONT_ID, y + 36, "Install Clear Reading Cache from the App Store or copy it to /Apps.");
