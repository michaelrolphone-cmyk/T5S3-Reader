...
void FontDownloadActivity::loop() {
  if (!launchAttempted) {
    ...
    const esp_err_t result = runNativeApp("/sd/Apps/font_manager.elf", renderer, mappedInput);
    if (result == ESP_OK) {
      finish();
      return;
    }
...
bool FontDownloadActivity::onTouchTap(int16_t x, int16_t y) {
...
void FontDownloadActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Manage Fonts");
  const int y = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing * 3;
  if (launchFailed) {
    renderer.drawCenteredText(UI_10_FONT_ID, y, "font_manager.elf could not be launched");
    renderer.drawCenteredText(SMALL_FONT_ID, y + 36, "Install Manage Fonts from the App Store or copy it to /Apps.");
