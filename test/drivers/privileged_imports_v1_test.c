...
int main(void)
{
    ...
    assert(esp_elf_privileged_imports_valid_v1(bytes, sizeof(bytes)));
    ...
    assert(!esp_elf_privileged_imports_valid_v1(bytes, sizeof(bytes)));
    assert(!esp_elf_privileged_imports_valid_v1(NULL, sizeof(bytes)));
    puts("Privileged provider ELF import preflight: allowlist, missing symbols and bounds PASS");
    return 0;
}
