...
namespace {
constexpr const char* ids[] = {"class.one", "class.two", "class.three"};
...
bool nextProvider(const char* cap, uint32_t api, size_t* cursor, char* id, size_t capacity) {
  ...
  if (!cursor || !id || capacity < 16 || *cursor >= 3) return false;
...
int main() {
  ...
  assert(cursor == 3 && acquisitions == 3 && releases == 3 && !selected.grant.slot);
  ...
  std::puts("Generic installed-provider selector ownership tests passed");
}
