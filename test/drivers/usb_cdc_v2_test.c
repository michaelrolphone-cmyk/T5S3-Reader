...
static int32_t control(void *ctx,uint64_t device,uint8_t type,uint8_t request,uint16_t value,uint16_t iface,uint8_t *payload,uint16_t length,uint32_t timeout){(void)ctx;assert(device&&type==0x21&&iface==0&&timeout==1000);assert((request==0x20&&payload&&length==7)||(request==0x22&&!payload&&length==0));last_request=request;last_value=value;if(payload)memcpy(last_payload,payload,7);++controls;return length;}
...
int main(int argc,char **argv){
 ...
 risc_usb_host_discovery_v1 host={{RISC_USB_HOST_API_V1,sizeof(host),0,configuration,claim,release_claim,control,bulk_read,bulk_write},0,0,checked_release};dep.api=&host.host;assert(driver->start(&dep,1)&&!driver->start(&dep,1));
 ...
 uint64_t next=cdc->open(2);assert(next&&next!=first&&!cdc->control_lines(first,true,false));driver->stop();assert(releases==6&&cdc->read(next,data,2,15)==-1);assert(driver->start(&dep,1));assert(!cdc->close(next));driver->stop();assert(dlclose(lib)==0);puts("CDC checked release, retry, I/O and stale token: PASS");return 0;
}
