...
cc -std=c11 -Wall -Wextra -Werror -fPIC -fvisibility=hidden -shared \
  -I"$repo/sdk/driver" "$repo/Drivers/bq25896/driver.c" \
  -o "$build/power.so"
cc -std=c11 -Wall -Wextra -Werror -fPIC -fvisibility=hidden -shared \
  -I"$repo/sdk/driver" "$repo/Drivers/t5s3_usb_power_profile/driver.c" \
  -o "$build/profile.so"
...
"$build/chain-test" "$build/i2c.so" "$build/clock.so" "$build/power.so" "$build/profile.so"
