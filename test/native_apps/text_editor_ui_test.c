...
static unsigned polls, acquisitions, labels, diagnostic_lines;
static bool inside;
...
static void label(int32_t x, int32_t y, int32_t w, const char *value) {
    ...
    if (strstr(value, "VBUSREF") || strstr(value, "preflight-conflict"))
        ++diagnostic_lines;
}
...
static bool acquire(const char *name, uint32_t version,
                    t5_provider_capability_lease_t *lease, const void **api) {
...
static bool last_error(char *buffer, size_t capacity) {
    const char message[] = "VBUSREF failure=preflight-conflict status=0x56 adc=0x97 power=0x16 | USBCTRL start-failed stage=vbus-acquire";
    return snprintf(buffer, capacity, "%s", message) < (int)capacity;
}
...
static t5_provider_capability_api_v1 fake_providers = {
    .api_version=T5_PROVIDER_CAPABILITY_API_VERSION,
    .struct_size=sizeof(t5_provider_capability_api_v1),
    .acquire=acquire, .release=release, .last_error=last_error
};
...
int main(void) {
    for (unsigned landscape = 0; landscape < 2; ++landscape) {
        ...
        inside = true; polls = acquisitions = labels = diagnostic_lines = 0;
        app_main();
        assert(acquisitions == 2 && labels >= 3 && diagnostic_lines >= 1);
        inside = false; polls = acquisitions = 0;
        app_main(); assert(!acquisitions);
    }
    // An older firmware supplies only the original ABI prefix. Do not read
    // past struct_size or refuse to launch merely because diagnostics are new.
    fake_providers.struct_size = offsetof(t5_provider_capability_api_v1, last_error);
    test_width = 540; test_height = 960;
    inside = true; polls = acquisitions = 0;
    app_main(); assert(acquisitions == 2);
