...
class ManifestTest(unittest.TestCase):
    ...
    def test_reject_invalid(self):
        valid = dict(display_name='App', file_name='app.elf', min_firmware_version='1.1.5',
                     version='1.0.0', icon='solid:f013')
        for field, value in [('file_name', '../app.elf'), ('file_name', 'other.elf'),
                             ('display_name', ''), ('display_name', 'x'*96),
                             ('min_firmware_version', '1.1.5-rc'),
                             ('version', ''), ('version', '1.0'), ('version', '1.0.0-rc'),
                             ('version', '1.2.65536'), ('icon', 'solid:110000'),
    ...
    def test_reject_missing_version(self):
        with tempfile.TemporaryDirectory() as directory:
            source = Path(directory) / 'app.c'
            source.with_suffix('.json').write_text(json.dumps(dict(
                display_name='App', file_name='app.elf', min_firmware_version='1.1.5', icon='solid:f013')))
