...
`test/programmer/esp_rom_protocol_test.cpp` checks bounded SLIP framing and escaping, malformed input, checksum and ROM status parsing. `test/programmer/esp_rom_session_test.cpp` checks initial absence versus pre-READY claimed identity, detachment, rapid replacement, explicit revocation, generic provider errors, and terminal versus temporary serial stream conditions. **`test/programmer/esp_rom_provider_test.cpp` compiles the production `NativeEspRomBridge.cpp` itself** against test-only Arduino/watchdog/deterministic-MD5 shims and fault-injected semantic serial and firmware-stream APIs. It exercises a complete simulated 64-KiB flash and 64-block verification/reset sequence; source truncation, hashing cancellation, detach during SYNC, erase and write, reset-control I/O/disconnect, mismatched target MD5, optional `FLASH_END` acknowledgment, reentrant `BUSY`, missing-context `DENIED`, invalid inputs, and exactly-once serial lease release on acquired terminal paths. The deterministic digest is a fixture, not a cryptographic MD5 test. All these tests run from `test/run_driver_test.sh`.

`test/streams/usb_device_registry_test.cpp` checks epoch monotonicity across redundant removal, identical replug and descriptor/interface changes; `test/streams/bridge_test.cpp` checks stale lease/stream rejection, replacement acquisition and app-unload cleanup. `test/native_apps/serial_monitor_test.c` exercises application-level recovery with fresh handles and no stale TX. CI also compiles both firmware board targets, native apps, driver packages, and manifest checks. These are host/build checks, **not physical programming tests**.
