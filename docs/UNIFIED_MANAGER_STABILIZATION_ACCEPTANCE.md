...
The live download intake now serializes refresh/install mutations, snapshots the selected catalog entry, checks the installed verified version and recovery state before network transfer, and refuses an existing `/Drivers/.driver-manager.part` rather than deleting an interrupted or unknown file. Same-version and downgrade candidates do not start a download. An early mapped-driver check also refuses installation **before** Wi-Fi/download when the target generation is already pinned. The publication-time exclusive lease remains authoritative if a loader races the early check. The shared downloader refuses existing `.part` files for native streams and the HTTPClient fallback; the fallback uses exclusive create (`O_EXCL`) rather than silently truncating an interrupted stage.
...
- Provide deliberate inspect/retry/discard UI for an interrupted `.part` or `.pkg-stage`; a refused operation currently preserves it for inspection but cannot recover it through the Driver Manager UI.

## Device test protocol

1. Record the exact PR head firmware commit, board type, SD payload versions and boot serial log. Back up the SD card before testing recovery. Do not attribute a PR #79 behavior to an older `master` build.
...
4. Attempt same-version and downgrade installs through the firmware API: reject before downloading, preserve installed bytes and leave no new stage. Plant a backed-up sentinel `/Drivers/.driver-manager.part`: installation must refuse without deleting/changing it; remove this sentinel manually only after inspection. An existing `.pkg-stage` must also be retained, not overwritten. Repeat the `.part` refusal while native streams are unavailable so that the HTTPClient fallback is exercised, checking that its exclusive create does not truncate the sentinel.
5. Interrupt or corrupt the next staged update: the old verified generation must remain usable and an unfinished stage must not be auto-deleted. An installed package with corrupt ELF/manifest must be refused, not treated as a fresh writable target. Manually inspect any stage on a backed-up SD; no recovery UI exists in this slice.
6. With a mapped driver active, attempt an update: early intake must refuse *before* network transfer. Race a new mapping after the early inspection: publication must still refuse without renaming active files. Stop the driver and repeat the versioned upgrade. Reboot with a pending old/new-format backup and inspect recovery behavior.
