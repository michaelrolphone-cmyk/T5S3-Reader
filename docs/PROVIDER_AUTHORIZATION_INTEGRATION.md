...
**Implementation state:** The provider authorization validator and GNSS integration are developed together in draft PR #67 against `master`. The GNSS semantic `subscribe()` and `poll()` paths use the common authorization validator. This is **not** complete hardware isolation or a sandbox: legacy `T5GpsApi`, `T5UsbApi`, direct USB streams and `T5SerialPortApi` v1 retain their existing access behavior; delegated GNSS record handles are not yet independently bound to consent revocation.
...
## GNSS provider integration — implemented in PR #67
