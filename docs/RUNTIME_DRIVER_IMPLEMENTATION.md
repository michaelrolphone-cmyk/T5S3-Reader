...
The registry now has a **24-entry fixed-capacity lifecycle journal**. Device registration emits `Added`; meaningful state transitions emit `StateChanged`; AVAILABLE/BUSY becoming unusable or removed also emits `CapabilityLost` with the number of forcibly revoked leases; removal emits `Removed` with a copied identity even after the device handle is invalid. Repeated identical states emit nothing. Each record has a monotonically increasing 64-bit sequence, and independent owner-task readers use `cursor()`/`poll()` without installing callbacks. `Gap` explicitly reports overwritten events, advances to the oldest retained boundary, and requires a current inventory resnapshot before resuming; `overwrittenEvents()` gives diagnostics. A cursor of zero can replay retained history but does not imply complete historical delivery. This is **pull-only firmware infrastructure**, not a globally scheduled discovery loop, push notification fabric, or app-facing subscription API. USB host task callbacks continue to publish snapshots only; they never mutate this journal.
...
`test/resources/device_registry_test.cpp`, `test/resources/device_event_test.cpp`, `test/resources/usb_serial_projection_test.cpp` and `test/streams/usb_direct_ownership_test.cpp` exercise identity, priority, conflicts, cross-owner denial, stale-generation rejection, app cleanup, journal ordering/loss/overflow, actual USB projection events, USB detach/replug, direct-versus-serial exclusivity, duplicate-open ownership and safe stale-stream rejection. The production USB semantic bridge and original stream test remain in the host suite. These tests do **not** prove physical USB enumeration, cross-core timing, UART/PSRAM/power behavior; those require on-device verification.

## Required convergence with the roadmap

The two initial live adapters are GNSS and USB serial. USB projection and lifecycle journal publication currently occur during native application owner-task operations, **not** through a global device-manager event loop. The journal does not notify subscribers, authenticate ELF event access, perform hardware probing, or enumerate unbound USB devices on its own. The USB host still has its existing lifecycle and compatibility snapshot; device registration is not evidence that its installable driver was activated or tested. Physical USB VBUS lifetime, multiple interfaces, hub discovery, BLE inventories, device lifecycle event fanout and independent driver/provider activation remain outstanding. The remaining `usbOpen` flag is a compatibility guard for stream slot occupancy; it must not be treated as the physical authorization source. This slice consolidates the known direct/serial paths' exclusive capability lease, not every future USB class or kernel client.
...
Next: verify physical direct/serial USB ownership and repeated disconnect/reconnect on-device; promote the registry to a runtime-wide discovery service with bounded event delivery based on the journal; add USB classes and BLE transport adapters; generalize resource cleanup and manifest capability requirements/launch gating; and expose semantic location without breaking deployed `T5GpsApi`. Network should migrate from fixed compiled binding to the same provider/capability model where feasible.
