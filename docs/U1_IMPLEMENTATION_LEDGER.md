...
- `test/run_stream_test.sh` now includes `usb_class_stream_session_test.cpp` (ASan/UBSan) plus prior stream/bridge tests.
- Firmware image not built in this increment. Physical USB/VBUS not exercised.

## Remaining U1

1. Retarget `NativeSerialPortBridge` acquire/control off `T5UsbApi` onto `ClassStreamSession` when a class ELF is bound; remove firmware USB data plane leftovers (`NativeUsbBridge` / resident CDC runtime fallback).
...
Wire `NativeSerialPortBridge` to `ClassStreamSession` so Serial Monitor/programmer consume class-ELF streams instead of `t5_usb_get_api`.
