...
## Device-registry diagnostic counters (implemented in PR #62)

`NativeUsbDevices::Registry::diagnostics()` takes a coherent, cross-core-protected snapshot containing the current claimed device/ID, revocation epoch, total bindings, total revocations, last revocation cause, and independent counts for host detach, loss of connection, host stop and binding identity change. Counters persist across app invocations and saturate rather than wrapping. Redundant detach and repeated READY polls must not add a false transition. `test/streams/usb_device_registry_test.cpp` exercises each category, same-identity replug and non-resetting generations.

**Scope limitation:** these counters measure claimed-interface transitions only. They do not capture failed pre-claim enumeration, host power timings, why an ELF probe failed, stream read/write failures, ROM commands or actual hardware timing. The snapshot is currently an internal firmware diagnostic accessor, not an ELF API or an automatic serial-log export. Record those separate layers with the original raw logs or external instrumentation. An absent event is *unobserved*, not proof that it did not occur.
