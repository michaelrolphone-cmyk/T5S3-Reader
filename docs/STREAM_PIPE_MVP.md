...
**Current branch implementation:** `src/runtime/streams/RecordQueue.h` introduces a reusable firmware-owned queue intended to reside inside the existing stream registry, not a competing public registry. It copies a validated, versioned ASCII schema string into firmware-owned storage; requires exact source/destination schema identity unless a future explicit transform mediates; allocates a bounded FIFO of up to eight records of up to 512 bytes each (at most 4096 payload bytes); and accepts or delivers a whole record atomically. Full queues return `AGAIN` without loss. Too-small read buffers leave the record untouched. Zero-byte records are distinguishable from `AGAIN` by the operation result. Buffered records drain before EOF/disconnect/error. The primitive tracks record and byte counts and high-water marks and clears state on explicit reset. Its tests are `test/streams/record_queue_test.cpp`, invoked by `test/run_stream_test.sh` with sanitizers.

**Integration still outstanding:** `RecordQueue` is not yet a stream kind in `StreamRuntime::Registry`; its compatibility predicate is not yet called by `Registry::connect`; the pipe scheduler still transfers bytes only; and no record constructor/read/write/metadata API is exported to ELFs. Before calling record streams functional, embed the queue in registry stream slots, bind schema and rights to execution-context-owned generation-safe handles, reject bytes↔records or unequal record schemas before allocating a pipe, transfer one record atomically per scheduler turn without copying a partial prefix into a sink, append a versioned ABI without changing v1 layout, and test unplug/unload, queue pressure, errors, stale handles and two concurrent pipes. Only then attach a real packet/sensor/GNSS producer. Do not represent these next steps as complete simply because the queue tests pass.

**Later full-spec phases:** introduce declaratively typed transforms and secure provider lifecycle; tee with explicit coupled/independent branch policies; ordered record merge with source identity/timestamps; additional overflow policies and drop counters; event-driven readiness/watermarks; stable discovery and resource quotas; record-oriented transport providers; and optional zero-copy, replay/ranges and persistent topology. Neither PR #65 nor the original byte MVP claims these are implemented.
...
`src/runtime/streams/StreamRuntime.cpp` owns at most 12 streams and four pipes. Memory FIFO sizes are 1–4096 bytes; staging and individual pipe transfers are bounded to 512 bytes. Only byte streams and the lossless `BLOCK_PRODUCER` pipe policy are currently exposed by the registry. A full destination applies backpressure rather than dropping bytes. Each active pipe exclusively leases the source read side and destination write side; self-pipes and cycles are rejected. `AGAIN`, EOF, disconnect, timeout, permission failure, stale/invalid handle, busy, cancellation, resource exhaustion, and I/O errors are distinct results.
...
Registry-connected record streams with versioned schemas, transforms, tee/merge, explicit drop policies and diagnostics, zero-copy pools, range/replay, discoverable logical topology, provider-ELF registration, per-manifest permissions, unified resource accounting, package-manager installation and stronger HTTP worker cancellation remain outstanding. In particular, stream payload transfer does not by itself supply package integrity/hash verification or certificate trust; those belong to the appropriate platform services. The App Store still has a separate UI limitation: the firmware indexes up to 128 release ELF assets while its native UI renders at most 64 rows.
