...
- verifying public HTTPS endpoints with the firmware certificate bundle when the caller does not supply a service-specific CA certificate;
...
The current application ABI still owns service-specific endpoint/method/authentication/body/serialization, optional service-specific certificate selection, HTTP-status policy, response parsing, retries, and application-level error handling. When `cert_pem` is null or empty for an HTTPS URL, the compatibility host uses the firmware's public root certificate bundle; supplying `cert_pem` overrides that default for the request. This describes current implementation, not the final roadmap boundary: reusable connection lifecycle, TLS policy, retries, downloads, and common protocol mechanics should migrate into RiscRTE platform services where they are not inherently application-specific.
